/*
 Fixed byteorder issues, implemented CBC mode, added proper padding algorithm.
 Output is now compatible with Perl and Java Blowfish+CBC libraries.
 -Aaron Wirtz
 ...
 http://www.ugpl.de/
*/
function Blowfish(k,iv){
        if (k.length<8) throw "key too short";
        this.iv = typeof(iv)=='undefined'?'0000000000000000':iv;
        ...
        k=this.unescape(k);k=(k.length>56)?k.substr(0,56):k;
        var j=0;
        for(var i=0;i<18;++i){
                var d=((k.charCodeAt(j%k.length)*256+k.charCodeAt((j+1)%k.length))*256+k.charCodeAt((j+2)%k.length))*256+k.charCodeAt((j+3)%k.length);
                this.bf_P[i]=this.xor(this.bf_P[i],d);
                j=(j+4);
        }
        this.key=this.escape(k);
...
Blowfish.prototype.encrypt=function(t){
        var pad=8-t.length%8;
        var t=this.escape(t);
        for(var i=0;i<pad;i++) t+='0'+pad;
        var r='';
        this.cr = this.wordunescape(this.iv.substr(8,8));
        this.cl = this.wordunescape(this.iv.substr(0,8));
        for(var i=0;i<t.length;i+=16){
                this.xr_par=this.xor(this.wordunescape(t.substr(i+8,8)),this.cr);
                this.xl_par=this.xor(this.wordunescape(t.substr(i,8)),this.cl);
                this.encipher();
                r+=this.wordescape(this.cl=this.xl_par)+this.wordescape(this.cr=this.xr_par);
        }
        return r;
};
Blowfish.prototype.decrypt=function(t){
        ...
        this.cl = this.wordunescape(this.iv.substr(0,8));
        for (var i=0;i<t.length;i+=16){
                var new_cr=this.xr_par=this.wordunescape(t.substr(i+8,8));
                var new_cl=this.xl_par=this.wordunescape(t.substr(i,8));
                this.decipher();
                r+=this.wordescape(this.xor(this.xl_par,this.cl))+this.wordescape(this.xor(this.xr_par,this.cr));
                this.cr=new_cr;this.cl=new_cl;
        }
        r=this.unescape(r);
        return r.substr(0,r.length-r.charCodeAt(r.length-1)%8);
};
